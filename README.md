...
| `OPEN_URL`    | Opens the URL in the default browser                                                    |
| `PLAY_AUDIO`  | Plays a `.wav` file (relative to the exe folder)                                        |
...
Animation scripts are used when the speech engine dispatch chain fails (see below).  
When `[EMULATE-ASYNC] OK` or `[EMULATE-STOP] OK` appears in the log, the game's own full handler ran and played its built‑in animations.
...
This bundles .NET 8 into the output so end‑users do **not** need a separate runtime installed.  
The finished `PAIcomPatcher.exe` is written to the `publish\` sub‑folder.
