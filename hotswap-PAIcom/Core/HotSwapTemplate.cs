        ...
        public static class HotSwapRuntime
        {
            ...
            public static void StartWatcher()
            {
                ...
                try
                {
                    ...
                    // Launch the command launcher UI on a background thread
                    var uiThread = new System.Threading.Thread(LaunchCommandLauncherUI)
                    {
                        IsBackground = true,
                        Name = "HotSwapUILauncher",
                        ApartmentState = System.Threading.ApartmentState.STA  // Required for WinForms
                    };
                    uiThread.Start();
            ...
            // ── UI Launcher (for in-game command UI) ───────────────────────

            /// <summary>
            /// Launches the command launcher UI form in a separate process.
            /// This runs asynchronously so it doesn't block game startup.
            /// </summary>
            private static void LaunchCommandLauncherUI()
            {
                try
                {
                    ...
                    foreach (var path in possiblePaths)
                    {
                        ...
                        {
                            launcherPath = path;
                            Log("[UI] Found launcher at: " + Path.GetFullPath(path));
                            break;
                        }
                    }
                    ...
                    Log("[UI] Launcher process started (PID: " + (proc?.Id ?? -1) + ")");
                }
                catch (Exception ex)
                {
                    Log("[UI] Failed to launch UI: " + ex.Message);
                }
            }
